...
`user_activity`, `memory`, `search`, and `meeting_lookup` are documented canonically in the `using-glean` skill (in the `glean-core` plugin):
...
- [`reference/memory.md`](../../../../glean-core/skills/using-glean/reference/memory.md) — `memory` + `memory_schema` for personalization
